...
namespace ours::mem {
    static Pte KERNEL_PAGE_TABLE_ENTRIES[X86PagingConfig::PAGE_LEVEL][X86PagingConfig::NR_PTE];

    template <>
    auto down_cast<X86MmuFlags>(MmuFlags flags) -> X86MmuFlags
    {
        X86MmuFlags arch_flags{0};
    ...
    // Normal page table.
    template <>
    auto X86PageTableImpl<X86PagingConfig>::
    ...
    auto X86PageTableImpl<PagingConfig>::
    ...
    create_mapping_at_level_0(Pte *table, MappingContext *context) -> Status
    {
        ...
        for (; index <= PagingConfig::NR_PTE; ++index) {
