...
namespace ours::mem {
    struct PhysMap {
        CXX11_CONSTEXPR
        static PhysAddr const PHYS_BASE = OURS_CONFIG_PHYS_MAP_PHYS_BASE;

        CXX11_CONSTEXPR
        static VirtAddr const VIRT_BASE = OURS_CONFIG_PHYS_MAP_BASE;

        CXX11_CONSTEXPR
        static usize const MAPPING_SIZE = OURS_CONFIG_PHYS_MAP_SIZE;

        CXX11_CONSTEXPR
        static auto is_valid_phys_addr(PhysAddr phys_addr) -> bool
        {  return PHYS_BASE <= phys_addr && phys_addr - PHYS_BASE < MAPPING_SIZE; }

        CXX11_CONSTEXPR
        static auto is_valid_virt_addr(VirtAddr virt_addr) -> bool
        {  return VIRT_BASE <= virt_addr && virt_addr - VIRT_BASE < MAPPING_SIZE; }
